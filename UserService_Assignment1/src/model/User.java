...
public class User {
	...
	private Connection connect() {
		...
		try {
			...
			con = DriverManager.getConnection(
					"jdbc:mysql://127.0.0.1:3306/user_service?useSSL=false&allowPublicKeyRetrieval=true", "root",
					"1234");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String readUsers(String type) {
		...
		try {
			...
			if (type.equals("consumer") || type.equals("Consumer")) {
				output = "<table border='1'><tr><th>User Name</th>" + "<th>Password</th>" + "<th> Gmail</th>"
						+ "<th>Address</th>" + "<th>DOB</th>" + "<th>phone</th>" + "<th>update</th>"
						+ "<th>Delete</th></tr>";

				String query = "select distinct c.userID,c.userCode,u.nKey as userName,p.nKey as password,c.email,c.address,c.dob,c.phone from consumer c,h_username u, h_password p where c.userName = u.nvalue and c.password = p.nvalue";
				...
				while (rs.next()) {
					...
					output += "<td><input name='btnUpdate' type='button' value='Update' "
							...
							+ "class='btnRemove btn btn-danger' data-usercode='" + consumerID + "'></td></tr>";
				}
				con.close();
			...
			if (type.equals("manufacturer") || type.equals("Manufacturer")) {
				...
				output = "<table border='1'><tr><th>User Name</th>" + "<th>Password</th>" + "<th> Gmail</th>"
						+ "<th>Address</th>" + "<th>DOB</th>" + "<th>phone</th>" + "<th>Description</th>"
						+ "<th>update</th>" + "<th>Delete</th></tr>";

				String query = "select distinct c.manufacturerID,c.manufacturerCode,u.nKey as userName,p.nKey as password,c.email,c.address,c.dob,c.phone,c.desc from manufacturer c,h_username u, h_password p where c.userName = u.nvalue and c.password = p.nvalue";
				Statement stmt = con.createStatement();
				ResultSet rs = stmt.executeQuery(query);
				...
				while (rs.next()) {
					...
					output += "<td><input name='btnUpdate' type='button' value='Update' "
							...
							+ "<td><input name='btnRemove' type='button' value='Remove' "
							+ "class='btnRemove btn btn-danger' data-usercode='" + manufacturerID + "'></td></tr>";
				}
				con.close();
				// Complete the html table
				output += "</table>";
			...
			if (type.equals("researcher") || type.equals("Researcher")) {
				...
				// Prepare the html table to be displayed
				output = "<table border='1'><tr><th>User Name</th>" + "<th>Password</th>" + "<th> Gmail</th>"
						+ "<th>Address</th>" + "<th>DOB</th>" + "<th>phone</th>" + "<th>profileInfo</th>"
						+ "<th>update</th>" + "<th>Delete</th></tr>";

				String query = "select distinct c.researcherID,c.researcherCode,u.nKey as userName,p.nKey as password,c.email,c.address,c.dob,c.phone,c.profileInfo from researcher c,h_username u, h_password p where c.userName = u.nvalue and c.password = p.nvalue";
				Statement stmt = con.createStatement();
				ResultSet rs = stmt.executeQuery(query);
				...
				while (rs.next()) {
					...
					output += "<td><input name='btnUpdate' type='button' value='Update' "
							...
							+ "<td><input name='btnRemove' type='button' value='Remove' "
							+ "class='btnRemove btn btn-danger' data-usercode='" + researcherID + "'></td></tr>";
				}
				con.close();
				// Complete the html table
				output += "</table>";
	...
	public String insertUser(String username, String password, String email, String address, String dob, String phone,
			String type, String desc, String profileInfo) {
		String output = "";
		try {
			Connection con = connect();
			if (con == null) {
	...
	public String updateUser(String type, String userID, String userName, String password, String email, String address,
			String dob, String phone, String desc, String profileInfo) {
		String output = "";
		try {
			Connection con = connect();
			if (con == null) {
	...
	public String deleteUser(String userID) {
		...
		try {
			 ...
			 * "delete from manufacturer where manufacturerID=?"; PreparedStatement
			 * preparedStmt = con.prepareStatement(query); // binding values
			 * preparedStmt.setInt(1, Integer.parseInt(userID)); // execute the statement
			 * preparedStmt.execute(); con.close(); // output =
			 * "Manufacturer deleted successfully";
			 * 
			 * 
			 * String newUser = readUsers(type); output =
			 * "{\"status\":\"success\", \"data\": \"" + newUser + "\"}";
			 * 
			 * }
			 *//********************* delete researcher ***********************//*
																				 * if (type.equals("researcher") ||
																				 * type.equals("Researcher")) {
																				 * 
																				 * // create a prepared statement String
																				 * query =
																				 * "delete from researcher where researcherID=?"
																				 * ; PreparedStatement preparedStmt =
																				 * con.prepareStatement(query); //
																				 * binding values preparedStmt.setInt(1,
																				 * Integer.parseInt(userID)); // execute
																				 * the statement preparedStmt.execute();
																				 * con.close(); // output =
